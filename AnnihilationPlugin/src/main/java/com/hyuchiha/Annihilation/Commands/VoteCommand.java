...
public class VoteCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    if (!VotingManager.isRunning()) {
      player.sendMessage(Translator.getPrefix() + ChatColor.RED +
          Translator.getColoredString("INFO.COMMAND_VOTING_ENDED"));
    } else if (args.length == 0) {
      listMaps(player);
    } else if (!VotingManager.vote(sender, args[0])) {
      player.sendMessage(Translator.getPrefix() + ChatColor.RED +
          Translator.getColoredString("INFO.COMMAND_VOTING_INVALID"));
      listMaps(player);
    }
    return true;
  }
  ...
  private void listMaps(Player player) {
    player.sendMessage(ChatColor.GRAY + Translator.getColoredString("INFO.COMMAND_VOTING_MAPS"));
    int count = 0;
